...
| 04 | Pennsic planner | container queries (`container-type: inline-size`, `@container`), `overflow: clip` vs scroll container, `position: sticky`, `<details>`/`<summary>` & attribute selectors, `var()` fallback & `hsl()` inline-property JS→CSS API, `-webkit-line-clamp` multi-line truncation, `scrollbar-width: none` | [04-pennsic-planner.md](./04-pennsic-planner.md) |
