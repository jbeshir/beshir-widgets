// golden.ts — authoritative conjugation oracle for the extended engine.
// Run: npx -y tsx test/golden.ts   (exits non-zero if ANY case is wrong.)
//
// Each case fixes a dictionary record (k, r, cls), a set of toggle options, and
// the expected FINAL (top) tier — kana, and for a representative subset the
// kanji surface. Expected forms are hand-verified standard 国文法 morphology and
// are INDEPENDENT of the engine implementation (a true oracle).

import { makeVerb, buildTower, type DictEntry, type TowerOpts } from '../src/engine';

type Opt = Partial<TowerOpts>;
const O = (o: Opt): TowerOpts => ({
  causative: !!o.causative,
  voice: o.voice ?? 'none',
  polite: !!o.polite,
  negative: !!o.negative,
  past: !!o.past,
...
interface Case {
  ...
  o: Opt;
  kana: string;   // expected top-tier kana
  kanji?: string; // expected top-tier kanji surface (subset)
}
...
const cases: Case[] = [
  ...
  // ── 帰る/入る/走る/切る godan-r (look-ichidan, are godan) ────────────────────
  ...
  // ── サボる godan-r (kana-only, katakana) ────────────────────────────────────
  ...
  // ── いらっしゃる godan-aru (polite い-stem) ──────────────────────────────────
  ...
  // ── 勉強する suru-noun (noun prefix fixed) ──────────────────────────────────
...
for (const c of cases) {
  const v = makeVerb({ k: c.k, r: c.r, romaji: '', cls: c.cls, common: true, gloss: 'x' });
  const tiers = buildTower(v, O(c.o));
  const top = tiers[tiers.length - 1];
  const optStr = JSON.stringify(c.o);
  if (top.kana !== c.kana) {
    fails.push(`KANA  ${c.k} ${optStr}: got「${top.kana}」want「${c.kana}」`);
  } else if (c.kanji !== undefined && top.kanji !== c.kanji) {
    fails.push(`KANJI ${c.k} ${optStr}: got「${top.kanji}」want「${c.kanji}」`);
  } else {
    pass++;
  }
}

console.log(`golden conjugation: ${pass}/${cases.length} pass, ${fails.length} fail`);
