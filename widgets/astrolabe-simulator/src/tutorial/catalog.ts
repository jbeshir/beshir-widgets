...
export const LESSONS = [
  ...
  },
  {
    ...
    steps: [
      ...
      step('find-date', 'Find the date on the calendar', 'On the back, find July 14 on the inner calendar ring. Its daily notches restart at 5, 10, 15, and so on within each unequal-width month.', 'back.calendar', base('back'), 'July 14 is located on the fixed 365-day calendar ring.'),
      step('read-sun-longitude', 'Read the Sun’s ecliptic longitude', `Rotate the alidade so the straight inner edge of one arm runs from the center through July 14. Continue across the zodiac-sign band to the degree scale on its outer side, where the edge meets about ${SUN_FIXTURE.eclipticLongitude.toFixed(1)}°. This is the Sun’s ecliptic longitude: its position around the ecliptic measured from 0° to 360°.`, 'back.ecliptic-longitude', base('back', { alidadeRotation: SUN_FIXTURE.eclipticLongitude }), `The alidade’s inner edge carries July 14 across the zodiac band to an ecliptic longitude of about ${SUN_FIXTURE.eclipticLongitude.toFixed(1)}°.`, { demonstration: { field: 'alidadeRotation', from: 0, to: SUN_FIXTURE.eclipticLongitude, durationMs: 700 } }),
