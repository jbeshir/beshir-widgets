...
const LIMB_TICKS = limbTicks();
const LIMB_HOURS = limbHourLabels();
  ...
  return (
    <svg className="astro-svg" role="img" aria-label={`Front face, latitude ${plateLatitude.toFixed(2)} degrees, rete ${reteRotation.toFixed(1)} degrees, rule ${ruleRotation.toFixed(1)} degrees`} viewBox={VIEWBOX}>
      ...
      <g transform="scale(1,-1)" data-tutorial-target="instrument">
        <g aria-label="Limb with 24 hour labels and one-degree subdivisions">
          {LIMB_TICKS.map((tick) => {
            const p1 = polar(outer - tick.inset, tick.degrees);
            const p2 = polar(outer - 2, tick.degrees);
            return <line key={tick.degrees} data-limb-degree={tick.degrees} className={`astro-limb-tick astro-limb-tick-${tick.level}`} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} />;
          })}
          {LIMB_HOURS.map((hour) => {
